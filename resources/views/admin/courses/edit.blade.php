    ...
    <div class="card-body">
        <form action="{{ route("admin.courses.update", [$course->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('price') ? 'has-error' : '' }}">
                ...
                <input type="number" id="price" name="price" class="form-control" value="{{ old('price', isset($course) ? $course->price : '') }}" step="0.01" min = "0">
                @if($errors->has('price'))
                    <em class="invalid-feedback">
                        {{ $errors->first('price') }}
                    </em>
            ...
            <div class="form-group {{ $errors->has('slots') ? 'has-error' : '' }}">
                <label for="slots">{{ trans('cruds.course.fields.slots') }}</label>
                <!--label for="slots">Slots*</label-->
                <input type="number" id="slots" name="slots" class="form-control" value="{{ old('slots', isset($course) ? $course->slots : '') }}" step="1" min = "0">
                @if($errors->has('slots'))
                    <em class="invalid-feedback">
                        {{ $errors->first('slots') }}
                    </em>
                @endif
                <p class="helper-block">
                    {{ trans('cruds.course.fields.slots_helper') }}
                </p>
            </div>
            <!-- -->
